...
- [`sdk/agentguard/`](sdk/agentguard/): core SDK modules. Highlights:
  ...
  - `instrument.py`, `decision.py`, `evaluation.py`, `cost.py`, `usage.py`, `savings.py`, `escalation.py`, `schemas.py`, `profiles.py`, `repo_config.py`, `reporting.py`, `export.py`: instrumentation, decision tracing, eval, cost/usage accounting, Anthropic thinking-token accounting, and report surfaces.
...
- Architecture knowledge is split between this root doc and [`ops/02-ARCHITECTURE.md`](ops/02-ARCHITECTURE.md), and on 2026-08-15 it drifted: `e346eb8c` added a "Current release state" section to the ops doc that this doc never received. The split now has a rule instead of a warning. **This root doc owns structure** - module boundaries, data flow, import direction, and the rules for adding features - and remains the law when the two disagree. **`memory/state.md` owns operating state** - SDK version, packaging and release status - as `AGENTS.md` already declares, with `memory/distribution.md` owning positioning and channels. `ops/02-ARCHITECTURE.md` holds dated architecture review notes and should cite `memory/` for anything versioned rather than restating it. Released version numbers are deliberately absent from this document, because copying them in is what makes a structural document go stale on a release cadence it does not control.
