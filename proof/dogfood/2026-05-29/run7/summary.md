# AgentGuard Dogfood Run 7 - 2026-05-29
...
- `agentguard doctor --json --trace-file proof/dogfood/2026-05-29/run7/doctor_trace.jsonl`
- `agentguard demo --trace-file proof/dogfood/2026-05-29/run7/demo_trace.jsonl`
- `set PYTHONPATH=./sdk&& python -c "import agentguard; ..."`
- `set PYTHONPATH=./sdk&& python -m agentguard.cli doctor --json --trace-file proof/dogfood/2026-05-29/run7/doctor_trace_repo_local.jsonl`
- `set PYTHONPATH=./sdk&& python -m agentguard.cli demo --trace-file proof/dogfood/2026-05-29/run7/demo_trace_repo_local.jsonl`
- `set PYTHONPATH=./sdk&& python examples/coding_agent_review_loop.py`
- `set PYTHONPATH=./sdk&& python -m agentguard.cli report proof/dogfood/2026-05-29/run7/demo_trace.jsonl`
- `set PYTHONPATH=./sdk&& python -m agentguard.cli report proof/dogfood/2026-05-29/run7/coding_agent_review_loop_traces.jsonl`
- `set PYTHONPATH=./sdk&& python -m agentguard.cli incident proof/dogfood/2026-05-29/run7/coding_agent_review_loop_traces.jsonl`
...
- Installed and repo-local doctor traces emitted `doctor.verify` start/end
  events.
...
See `guard_events.json` for extracted event data and the raw `*.jsonl` files
for complete trace evidence.
...
- `python -m pip install -e ./sdk` passed on rerun; the first capture hit a
  PowerShell stderr handling artifact even though pip reported successful
  installation.
- Focused proof and metadata tests passed: `35 passed in 1.38s`.
...
- Artifact JSON/JSONL parse, UTF-8, BOM, NUL, and local-path hygiene passed
  after normalizing PowerShell-captured output to UTF-8.
...
This run updates the existing dogfood proof PR #544 and rolling issue #490
rather than creating a duplicate dogfood PR.
