...
`.showwork/snapshots/*.json` is `text eol=lf -diff` in `.gitattributes`.
The JSON body stays out of `git diff` and `gh pr diff` so it does not fill
the review bot's 200k cap before SDK source. A later edit still shows up as
`Binary files differ`; it does not disappear. The session JSONL hash chain
is the content check. Snapshot bytes on disk stay the recorded hashes.
