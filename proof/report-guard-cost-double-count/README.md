...
`summarize_trace` (and therefore `incident`), `assert_cost_under`, and
`receipt` (via the per-event `_spend_cost`).
...
| `make-check-sdk.txt` | `ci-tools-guard review-readiness lint test`: 1306 passed, 92.32% coverage (after merging main through #792) |
