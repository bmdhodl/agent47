...
- Current source version: 1.4.1 candidate. It adds `agentguard --version`, `agentguard receipt` (#781), `agentguard hook claude-code` and `agentguard run` (#782), patches the OpenAI Responses API and so the Agents SDK (AG-06, Experimental in the compatibility matrix because the openai floor predates it), fixes `AsyncOpenAI`/`AsyncAnthropic` calls that crashed after `init()`, stops `report`, `summarize_trace`, and `receipt` double-counting the call that trips a budget, and ships the restored CrewAI/ChromaDB disclosure on PyPI. PyPI stays 1.4.0 until `v1.4.1` is tagged; the landing page (#784) already labels the three new commands "new in 1.4.1".
