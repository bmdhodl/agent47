"""Two processes, one remaining streamed call, one mock dispatch.

Store-backed OpenAI Chat Completions with ``stream=True`` reserve before
send. The other worker is refused before its mock runs. In-memory streams
still check and consume after the fact. No network. Spawn is the start
...
def _worker(store_path: str, log_path: str, barrier: multiprocessing.Barrier, queue: multiprocessing.Queue) -> None:
    ...
    def original(*_args: object, **_kwargs: object) -> _Stream:
        with open(log_path, "a", encoding="utf-8") as handle:
            handle.write("1\n")
        return _Stream()

    guard = BudgetGuard(max_calls=1, store=JsonFileStateStore(store_path), key="fleet")
    tracer = Tracer(sink=_Drop(), service="stream-reservation-race", watermark=False)
    try:
        barrier.wait(30)
        stream = _traced_openai_create(
            original, tracer, guard, model="gpt-4o-mini", stream=True
        )
        list(stream)
...
def main() -> dict:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        payload = {
            ...
            "network_calls": 0,
            "path": "openai-sync-stream-statestore",
            "fixed": (
                dispatched == 1
                and results.count("blocked") == 1
                and totals["settled"]["calls"] == 1
                and totals["reserved"]["calls"] == 0
            ),
        }
